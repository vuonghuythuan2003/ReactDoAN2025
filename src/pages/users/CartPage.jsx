...
import { fetchCartItems, updateCartItem, removeCartItem, clearCart } from '../../redux/reducers/CartSlice';
import HeaderUser from './HeaderUser';
import FooterUser from './FooterUser';
import { Card, Button, Form, Input, Row, Col, Typography, Space, Divider } from 'antd';
...
const CartPage = () => {
  ...
  const { items, loading, error, totalItems } = useSelector((state) => state.cart);
  ...
  useEffect(() => {
    ...
    if (user?.userId && totalItems === 0 && !loading) {
      console.log('CartPage: Fetching cart items for userId:', user.userId);
      dispatch(fetchCartItems(user.userId));
    }
  }, [isAuthenticated, user, dispatch, navigate, totalItems, loading]);
  ...
  const handleRemoveItem = (cartItemId) => {
    if (!isAuthenticated || !user?.userId) {
      toast.warning('Vui lòng đăng nhập để xóa sản phẩm!', { position: 'top-right', autoClose: 3000 });
      navigate('/login');
      return;
    }
  ...
  const handleClearCart = () => {
    if (!isAuthenticated || !user?.userId) {
      toast.warning('Vui lòng đăng nhập để xóa giỏ hàng!', { position: 'top-right', autoClose: 3000 });
      navigate('/login');
      return;
    }
    ...
    if (window.confirm('Bạn có chắc muốn xóa toàn bộ giỏ hàng không?')) {
      dispatch(clearCart(user.userId))
        .unwrap()
        .then(() => {
          toast.success('Đã xóa toàn bộ giỏ hàng!', { position: 'top-right', autoClose: 3000 });
        })
        .catch((error) => {
  ...
  const handleCheckout = async (values) => {
    if (!isAuthenticated || !user?.userId) {
      toast.warning('Vui lòng đăng nhập để thanh toán!', { position: 'top-right', autoClose: 3000 });
      navigate('/login');
      return;
    }

    try {
      const response = await BASE_URL_USER.post('/cart/checkout', null, {
        params: {
          userId: user.userId,
      ...
      toast.error('Thanh toán thất bại!', { position: 'top-right', autoClose: 3000 });
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '20px' }}>Đang tải giỏ hàng...</div>;
                  ...
                  <Col xs={24} md={20}>
                    <Row align="middle" style={{ padding: '0 20px' }}>
                      <Col xs={24} md={16}>
                        ...
                        <br />
                        <Text style={{ color: '#6c757d' }}>
                          Tổng: {((item.unitPrice || 0) * (item.quantity || 0)).toLocaleString('vi-VN')} VNĐ
                        </Text>
                      </Col>
                      <Col xs={24} md={8} style={{ textAlign: 'right' }}>
                        <Space>
                          ...
                          <Text style={{ fontSize: '1.1rem', fontWeight: 500 }}>{item.quantity}</Text>
                          <Button
                            type="primary"
                            onClick={() => handleQuantityChange(item.cartItemId, item.quantity + 1)}
                            style={{
