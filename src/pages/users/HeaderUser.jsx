...
const HeaderUser = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.userId && totalItems === 0 && !loading) {
      console.log('HeaderUser: Fetching cart items for userId:', user.userId);
      dispatch(fetchCartItems(user.userId));
    } else if (!isAuthenticated) {
      console.log('HeaderUser: Resetting cart - User not authenticated');
      dispatch(resetCart());
    }
  }, [isAuthenticated, user, dispatch, totalItems, loading]);
